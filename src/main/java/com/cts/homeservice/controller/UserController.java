...
public class UserController {
	...
	@RequestMapping(value="userRequestStatus.html", method=RequestMethod.POST)
	public ModelAndView getUserRequestStatusOnRequest(HttpSession session, HttpServletRequest request)
	{
		session = request.getSession(false);
		ModelAndView modelAndView = new ModelAndView();
		User user = (User)session.getAttribute("user");
	...
	public ModelAndView addServiceRequest(@ModelAttribute Requests requests, HttpSession session, HttpServletRequest request) {
		...
		if("success".equals(result)) {
			...
			modelAndView.setViewName("redirect:/userRequestStatus.html");
