...
public class SearchController {
	...
	public ModelAndView getServiceResult(HttpServletRequest request) throws JSONException, IOException, ParseException {
		...
		List<Worker> workers = workerService.getWorkerByLocalityAndService(vicinity,service);
		 HashMap<String, Integer> localityOccurance = new HashMap<>(); 
		 
		 for(Worker worker : workers) {
			 if(localityOccurance.containsKey(worker.getLocality())){
				 localityOccurance.put(worker.getLocality(),localityOccurance.get(worker.getLocality())+1);
			 }
			 else
				 localityOccurance.put(worker.getLocality(),1);
		 }
		
		modelAndView.addObject("numOfWorkers", workers.size());
		modelAndView.addObject("workerByLocality", localityOccurance);
